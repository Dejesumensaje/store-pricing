...
// when it trims it — so the trade-off reads at a glance.
function StatField({ label, value, projection }: { label: string; value: number; projection?: Projection | null }) {
...
// price change annotates Weekly units with its projected sales range.
export function ItemStats({ onHand, weekly, weeklyProjection }: Props) {
  return (
    <section className="flex gap-6">
      <StatField label="On hand" value={onHand} />
      <StatField label="Weekly units" value={weekly} projection={weeklyProjection} />
    </section>
  );
}
